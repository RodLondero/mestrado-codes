...
def main():
    ...
    plot_energia_incidente(EI_ASTM.copy(),    ax[1][0], title="Energia Incidente (ASTM)")
    ax_ei = ax[1,0] # type: axes.Axes
...
def plot_temperatura_fluido(file_name: str, ax: axes.Axes):
    ...
    ax = temperatura_fluido.plot(x='flow-time', y='Temperatura', ax=ax,
                                 ...
                                 xlim=(0, max(temperatura_fluido['flow-time'])),
...
def plot_temperatura_calorimetro(file_name:str, ax: axes.Axes, ls='solid'):
    temperatura_calorimetro = get_data_from_file(file_name)
